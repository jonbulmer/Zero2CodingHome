...
namespace Model
{
    ...
    public class StudentsWithGrades
    {
        ...
        private static string ConcatenateStudentExams(List<Grade> StudentsExams)
        {
            ...
            foreach (var item in StudentsExams)
            {
                ...
                var courseName = courseNames.FirstOrDefault(x => x.CourseId == item.CourseId).CourseName.ToString();
                result += courseName + "||";
            }
            return result;
        }

        public static bool HasPassed(List<Grade> studentsGrades)
        {
            bool result;
